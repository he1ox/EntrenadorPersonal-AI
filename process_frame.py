...
class ProcessFrame:
    ...
    def _get_state(self, knee_angle):
        ...
        if self.thresholds['HIP_KNEE_VERT']['NORMAL'][
                0] <= knee_angle <= self.thresholds['HIP_KNEE_VERT']['NORMAL'][
                    1]:
            knee = 1
        elif self.thresholds['HIP_KNEE_VERT']['TRANS'][
                0] <= knee_angle <= self.thresholds['HIP_KNEE_VERT']['TRANS'][
                    1]:
            knee = 2
        elif self.thresholds['HIP_KNEE_VERT']['PASS'][
                0] <= knee_angle <= self.thresholds['HIP_KNEE_VERT']['PASS'][1]:
            knee = 3
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            nose_coord = get_landmark_features(ps_lm.landmark,
                                               self.dict_features, 'nose',
            ...
            offset_angle = find_angle(left_shldr_coord, right_shldr_coord,
                                      nose_coord)
            ...
            if offset_angle > self.thresholds['OFFSET_THRESH']:
                ...
                self.state_tracker[
                    'INACTIVE_TIME_FRONT'] += end_time - self.state_tracker[
                ...
                if self.state_tracker[
                        'INACTIVE_TIME_FRONT'] >= self.thresholds[
                            'INACTIVE_THRESH']:
                ...
                cv2.circle(frame, left_shldr_coord, 7, self.COLORS['yellow'],
                           -1)
                cv2.circle(frame, right_shldr_coord, 7, self.COLORS['magenta'],
                           -1)
                ...
                draw_text(frame,
                          'REPS. CORRECTAS: ' +
                          str(self.state_tracker['SQUAT_COUNT']),
                ...
                draw_text(frame,
                          'REPS. INCORRECTAS: ' +
                          str(self.state_tracker['IMPROPER_SQUAT']),
            ...
            # La cámara está alineada correctamente.
                ...
                hip_vertical_angle = find_angle(shldr_coord,
                                                np.array([hip_coord[0], 0]),
                                                hip_coord)
                cv2.ellipse(frame,
                            hip_coord, (30, 30),
                            angle=0,
                ...
                draw_dotted_line(frame,
                                 hip_coord,
                                 start=hip_coord[1] - 80,
                                 end=hip_coord[1] + 20,
                                 line_color=self.COLORS['blue'])

                knee_vertical_angle = find_angle(hip_coord,
                                                 np.array([knee_coord[0], 0]),
                                                 knee_coord)
                cv2.ellipse(frame,
                            knee_coord, (20, 20),
                            angle=0,
                ...
                if current_state == 's1':
                    ...
                    ) == 3 and not self.state_tracker['INCORRECT_POSTURE']:
                        self.state_tracker['SQUAT_COUNT'] += 1
                        play_sound = str(self.state_tracker['SQUAT_COUNT'])

                    elif 's2' in self.state_tracker['state_seq'] and len(
                    ...
                    self.state_tracker[
                        'TOTAL_SQUAT_COUNT'] = self.state_tracker[
                            'SQUAT_COUNT'] + self.state_tracker[
                                'IMPROPER_SQUAT']
                ...
                # -------------------------------------- PERFORM FEEDBACK ACTIONS --------------------------------------
                    ...
                    elif knee_vertical_angle > self.thresholds['KNEE_THRESH'][
                            2]:
                        self.state_tracker['DISPLAY_TEXT'][3] = True
                        self.state_tracker['INCORRECT_POSTURE'] = True

                    if (ankle_vertical_angle
                            > self.thresholds['ANKLE_THRESH']):
                ...
                if self.state_tracker['curr_state'] == self.state_tracker[
                        'prev_state']:

                    end_time = time.perf_counter()
                    self.state_tracker[
                        'INACTIVE_TIME'] += end_time - self.state_tracker[
                            'start_inactive_time']
                    self.state_tracker['start_inactive_time'] = end_time

                    if self.state_tracker['INACTIVE_TIME'] >= self.thresholds[
                            'INACTIVE_THRESH']:
                        self.state_tracker['SQUAT_COUNT'] = 0
                        self.state_tracker['IMPROPER_SQUAT'] = 0
                        display_inactivity = True
                ...
                if 's3' in self.state_tracker[
                        'state_seq'] or current_state == 's1':
                    self.state_tracker['LOWER_HIPS'] = False

                self.state_tracker['COUNT_FRAMES'][
                    self.state_tracker['DISPLAY_TEXT']] += 1
                ...
                draw_text(frame,
                          'REPS. TOTALES: ' +
                          str(self.state_tracker['TOTAL_SQUAT_COUNT']),
            ...
            self.state_tracker['COUNT_FRAMES'] = np.zeros((5, ),
                                                          dtype=np.int64)
            self.state_tracker[
                'start_inactive_time_front'] = time.perf_counter()
